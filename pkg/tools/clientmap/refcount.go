...
//               each Delete decrements the count.  Each element will be Deleted from the map no later than when its
//               refcount is zero.  There is some potential for premature deletion, so RefcountMap should only
//               be used in cases that can tolerate strictly the 'no later than when its refcount is zero'
...
// Delete decrements the refcount and deletes the value for a key no later than when its refcount is zero.
